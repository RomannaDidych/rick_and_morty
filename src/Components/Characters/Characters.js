...
class Characters extends Component{
	...
	getChracters = async (e) =>{
		let arr;
		if (e.target.value === 'next')	{
			if (this.currentNumber > this.maxAmount) {
				return;
			}
			arr = this.getNextArr();
		...
		return (
			<div className='container'>				
				...
				<div className= "buttons">
					<button className= 'btn' onClick={this.getChracters} value='previous'>previous characters</button>
					<button className= 'btn' onClick={this.getChracters} value='next'>next characters</button>
				</div>
			</div>
			)
	}
